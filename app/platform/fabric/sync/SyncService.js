		...
		if (channel_genesis_hash) {
			...
			for (let i = 0; i < txLen; i++) {
				...
				if (txObj.payload.data.actions !== undefined) {
					// logger.info('Transaction Detail', JSON.stringify(txObj.payload.data.actions[0].payload.action.proposal_response_payload.extension.response.payload));
					// ascii, utf8, utf16le/ucs2, base64, binary, and hex
					// detail = JSON.stringify({ data : String.fromCharCode.apply(null, txObj.payload.data.actions[0].payload.action.proposal_response_payload.extension.response.payload) });
					detail = JSON.stringify(txObj.payload.data.actions[0].payload.action.proposal_response_payload.extension.response.payload);
